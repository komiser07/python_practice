...
def find_book(title):
    if title in library:
        ...
        print(f"Year of publication: {library[title]['year']}")
        print(f"Availability: {library[title]['availability']}\n")
        if library[title]['availability'] == 'в наличии':
            print(f"Книга '{title}' доступна\n")
        elif library[title]['availability'] == 'выдана':
